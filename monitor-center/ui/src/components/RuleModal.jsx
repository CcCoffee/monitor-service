...
const RuleModal = ({
  ...
  const generateServerOptions = () => {
    return fetch('http://localhost:8090/servers/all')
      ...
      .catch((error) => {
        console.error(
          'An error occurred while generating server options:',
          error
        )
  ...
  const querySelectedServerOptions = () => {
    if (rule.id) {
      fetch(`http://localhost:8090/servers/all?ruleId=${rule.id}`)
        ...
        .catch((error) => {
          console.error(
            'An error occurred while querying selected server options:',
            error
          )
          // TODO: 可以执行其他操作，如显示错误消息等
        })
  ...
  const querySelectedChannelOptions = () => {
    if (rule.id) {
      fetch(`http://localhost:8090/channels/all?ruleId=${rule.id}`)
        ...
        .catch((error) => {
          console.error(
            'An error occurred while querying selected channel options:',
            error
          )
          // TODO: 可以执行其他操作，如显示错误消息等
        })
      ...
      <Modal.Body>
        <Form>
          ...
          <Row>
            <Col>
              <Form.Group
                controlId="newRuleNotificationRecipients"
                className="mb-1">
                ...
                <Form.Control
                  ...
                  defaultValue={
                    rule.notificationRecipients
                      ? rule.notificationRecipients.join('\n')
                      : ''
                  }
                  onChange={handleRuleInputChange}
                  required
            ...
            <Col>
              <Form.Group controlId="newRuleLogPatterns" className="mb-1">
                ...
                <Form.Control
                  ...
                  name="logPatterns"
                  defaultValue={
                    rule.logPatterns ? rule.logPatterns.join('\n') : ''
                  }
                  onChange={handleRuleInputChange}
                />
              </Form.Group>
