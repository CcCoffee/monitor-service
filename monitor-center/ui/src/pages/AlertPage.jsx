...
const AlertPage = () => {
  ...
  const [showUpdateStatusConfirmation, setShowUpdateStatusConfirmation] =
    useState(false)
  const [updateAlertId, setUpdateAlertId] = useState()
  ...
  const handleShowUpdateStatusConfirmation = (alertId, nextStatus) => {
    setShowUpdateStatusConfirmation(true)
    setUpdateAlertId(alertId)
    setNextStatus(nextStatus)
  }

  const handleConfirmUpdateStatus = (status) => {
    const alertId = updateAlertId
    let url = `http://localhost:8090/alerts/${alertId}/status`
      ...
      .then((response) => {
        if (response.ok) {
          ...
          response.json().then((data) => {
            console.error(data.message)
            // TODO: 处理请求失败的情况，如显示错误消息等
            notifyFailure('Update status failed', data.message)
          })
        }
      })
      .catch((error) => {
    ...
    // 关闭确认框并重置服务器ID
    setShowUpdateStatusConfirmation(false)
    setUpdateAlertId(null)
  }

  const getUpdateStatusConfirmMessage = (status) => {
    if (status === 'Closed') {
      return 'Are you sure you want to close this alert?'
    } else {
      return 'Are you sure you want to acknowleage this alert?'
    }
  }
    ...
    <Container className="h-100 w-100 py-2">
      <Card className="h-100 w-100">
        <Card.Body>
                ...
                <tr key={alert.id}>
                  ...
                  <td>
                    <Button
                      size="sm"
                      variant="primary"
                      onClick={() => navigateToAlertViewPage(alert)}>
                      View
                    </Button>{' '}
                    {alert.status === 'Open' ? (
                      <Button
                        ...
                        variant="success"
                        onClick={() =>
                          handleShowUpdateStatusConfirmation(
                            alert.id,
                            'Acknowledged'
                          )
                        }>
                    ...
                    {alert.status === 'Acknowledged' ? (
                      <Button
                        ...
                        variant="danger"
                        onClick={() =>
                          handleShowUpdateStatusConfirmation(alert.id, 'Closed')
                        }>
                        Close
                      </Button>
                    ) : (
          ...
          {/* 状态更新确认框 */}
          <Modal
            show={showUpdateStatusConfirmation}
            onHide={() => setShowUpdateStatusConfirmation(false)}>
            ...
            <Modal.Body>
              <Alert variant="warning">
                <p className="mb-0">
                  {getUpdateStatusConfirmMessage(nextStatus)}
                </p>
              </Alert>
            </Modal.Body>
            <Modal.Footer>
              <Button
                variant="secondary"
                onClick={() => setShowUpdateStatusConfirmation(false)}>
                Cancel
              </Button>
              <Button
                variant="danger"
                onClick={() => handleConfirmUpdateStatus(nextStatus)}>
                Update
              </Button>
            </Modal.Footer>
          </Modal>
