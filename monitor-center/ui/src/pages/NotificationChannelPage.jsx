...
import { Table, Form, Button, Row, Col, Container, Modal, Alert, Card } from 'react-bootstrap';
...
const NotificationChannelPage = () => {
  ...
  const handleSaveChannel = async () => {
    console.log("handleSaveChannel", selectedChannel);
    selectedChannel.configuration = {
      emailAddress: selectedChannel.newEmailAddresses ?selectedChannel.newEmailAddresses.trim().split(","):selectedChannel.configuration.emailAddress,
      emailTemplate: selectedChannel.emailTemplate
    };
    console.log("handleSaveChannel", selectedChannel);
    try {
              ...
              <tbody>
                {filteredChannels.map(channel => (
                  <tr key={channel.id}>
                    ...
                    <td>
                      <Button variant="primary" onClick={() => handleEditChannel(channel.id)}>Edit</Button>{' '}
                      <Button variant="danger" onClick={() => handleDeleteChannel(channel.id)}>Delete</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <Modal show={showDeleteConfirmation} onHide={() => setShowDeleteConfirmation(false)}>
            ...
            <Modal.Body>
              <Alert variant="warning">
                <p>
                  Please make sure that the channel to be deleted is not being used by any rule.
                </p>
                <hr/>
                <p className="mb-0">
